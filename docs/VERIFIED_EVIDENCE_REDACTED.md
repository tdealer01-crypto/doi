# DSG Spacetime — Redacted Verified Evidence Summary

This page records public-safe verification outcomes. Internal workflow IDs, customer identifiers, secrets, private test mechanics, and implementation-sensitive details are intentionally omitted.

## Verified runtime scope

The private production test/release process has produced execution evidence for the following scoped behaviors:

- approved Node/capability discovery
- AI composition treated as untrusted until bound
- plan-bound Route execution
- deployment-bound signed entitlement verification
- licensed Route authorization
- fail-closed invalid deployment/license behavior
- local MCP execution surface
- customer-owned adapter boundary
- durable evidence recording and verification
- source-free Linux AMD64 customer packaging
- rejection of customer/self-issued commercial trust material

## Browser App verification scope — 2026-09-14

A private production change added a governed Browser App surface for bounded web missions. The associated private runtime CI and source-free packaging smoke checks passed for the integrated change.

This public statement verifies the source/CI/packaging scope only. It intentionally omits private workflow identifiers, implementation mechanics, security-control internals, and deployment topology. A provider deployment or live browser task is not inferred from these checks alone.

## Verified commerce acceptance scope

A controlled owner-authorized LIVE payment acceptance exercise verified the real provider chain at a deliberately low test amount. The proof covered payment completion, seller-signed entitlement issuance, exact delivered-package runtime smoke, deployment-mismatch blocking, transactional delivery, durable fulfillment recording, duplicate-retry suppression, and physical mailbox receipt of the expected delivery artifacts.

The controlled proof amount was **THB 10.00**. It must not be represented as a USD 299 commercial purchase.

## Claim boundary

These statements describe the tested scope only. They do not imply that every possible integration, deployment, legal regime, platform, or customer configuration has been independently verified.

Detailed implementation, raw private logs, private CI/test logic, credentials, customer data, and anti-bypass mechanisms remain outside this public repository.
